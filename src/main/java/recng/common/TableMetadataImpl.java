...
public class TableMetadataImpl implements TableMetadata {
    ...
    public TableMetadataImpl(List<FieldMetadata> fieldMetadata) {
        ...
        for (FieldMetadata field : fieldMetadata) {
            index.put(field.getFieldName().toUpperCase(), i++);
            fields.add(field);
        }
    }
    ...
    public FieldMetadata getFieldMetadata(String fieldName) {
        if (!index.containsKey(fieldName.toUpperCase()))
            return null;
        int ordinal = index.get(fieldName.toUpperCase());
    ...
    public boolean contains(String fieldName) {
        return index.containsKey(fieldName.toUpperCase());
    }
    ...
    public int ordinal(String fieldName) {
        if (!contains(fieldName))
            throw new IllegalArgumentException("Unknown field name: "
                + fieldName);
        return index.get(fieldName.toUpperCase());
    }
